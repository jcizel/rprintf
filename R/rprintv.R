...
rprintv <- function(x,...) {
  ...
  xs <- unlist(stringr::str_extract_all(x,stringr::regex("(?<!\\$)\\$[\\w\\._]+(:[\\s\\+\\-\\#\\.\\d]*\\w)?")))
  xss <- stringr::str_replace_all(xs,stringr::regex("(?<!\\$)\\$([\\w\\._]+)(:[\\s\\+\\-\\#\\.\\d]*\\w)?"),"\\1")
  pass1 <- stringr::str_replace_all(x,stringr::regex("(?<!\\$)\\$([\\w\\._]+):(?!\\$)(?!:+)([\\s\\+\\-\\#\\.\\d]*\\w)?"),"%\\2")
  pass2 <- stringr::str_replace_all(pass1,stringr::regex("(?<!\\$)\\$([\\w\\._]+)"),"%s")
